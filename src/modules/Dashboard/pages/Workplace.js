...
@inject(['dashboardProject', 'dashboardActivities', 'dashboardChart'])
@connect(({ dashboardProject, dashboardActivities, dashboardChart, loading }) => ({
  dashboardProject,
  dashboardActivities,
  dashboardChart,
  projectLoading: loading.effects['dashboardProject/fetchNotice'],
  activitiesLoading: loading.effects['dashboardActivities/fetchList'],
}))
export default class Workplace extends PureComponent {
  componentDidMount() {
    const { dispatch } = this.props;
    dispatch({
      type: 'dashboardProject/fetchNotice',
    });
    dispatch({
      type: 'dashboardActivities/fetchList',
    });
    dispatch({
      type: 'dashboardChart/fetch',
    });
  }
  ...
  componentWillUnmount() {
    const { dispatch } = this.props;
    dispatch({
      type: 'dashboardChart/clear',
    });
  }

  renderActivities() {
    const {
      dashboardActivities: { list },
    } = this.props;
  ...
  render() {
    const {
      dashboardProject: { notice },
      projectLoading,
      activitiesLoading,
      dashboardChart: { radarData },
    } = this.props;
